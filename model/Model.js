...
module.exports = (function() {
    ...
    function Model(defaults) {
        ...
        var privs = {
            ...
            buildGroups: function() {
                ...
                var width =
                    (attrs.get(self).size.getWidth() - (attrs.get(self).border.left + attrs.get(self).border.right)) /
                    attrs.get(self).cols;
                var height =
                    (attrs.get(self).size.getHeight() - (attrs.get(self).border.top + attrs.get(self).border.bottom)) /
                    attrs.get(self).rows;
            ...
            rebuildGroups: function() {
                ...
                for (let group of attrs.get(self).groups) {
                    ...
                    do {
                        ...
                        var y =
                            ((tiles[i].getY() - oldBottomBorder) / oldHeight) * height + attrs.get(self).border.bottom;
    ...
    Model.prototype.setSize = function(raw_size, doNotSave) {
        ...
        if (attrs.get(this).orientation == Orientation.LANDSCAPE) {
            attrs.get(this).size = new Rect(0, 0, Math.max(width, height), Math.min(width, height));
        } else {
            attrs.get(this).size = new Rect(0, 0, Math.min(width, height), Math.max(width, height));
        }
    ...
    Model.prototype.splitGroups = function(groups) {
        ...
        for (let group of groups) {
            ...
            for (let tile of tiles) {
                var new_group = new Group(new ImageData(image, tile), tile, [tile]);
                this.addGroup(new_group);
