...
module.exports = (function() {
    ...
    ContentView.prototype.notify = function(model, groups) {
        ...
        context.fillRect(0, 0, display_size.getWidth(), display_size.getHeight());
        // Draw the groups.
    ...
    ContentView.prototype.drawGroup = function(context, group, border, save) {
        ...
        for (let tile of tiles) {
            ...
            var width =
                tile.getWidth() -
                (group.isLeftMost(tile) ? border.left : 0) -
                (group.isRightMost(tile) ? border.right : 0);
            var height =
                tile.getHeight() -
                (group.isTopMost(tile) ? border.top : 0) -
            ...
            if (!save) this.drawSelectedDot(context, group, x, y, width, height, border);
        }
        for (let tile of tiles) {
    ...
    ContentView.prototype.drawEmptyGroup = function(context, group, border, save) {
        ...
        for (let tile of tiles) {
            ...
            context.fillRect(this.scale(x), this.scale(y), this.scale(width), this.scale(height));

            if (tile == tiles[0]) {
                // Draw the icon.
                context.fillStyle = "white";
                context.textAlign = "center";
                var fontSize = this.scale(Math.min(width, height) / 4);
                context.font = fontSize + "px Material Icons";
                context.fillText("photo_size_select_actual", this.scale(x + width / 2), this.scale(y + height / 2));
                // Draw the message.
                var message = "Right Click to Select";
                context.font = Math.min(fontSize / 2, this.scale(width) / message.length) + "px Roboto";
                context.fillText(message, this.scale(x + width / 2), this.scale(y + height / 2) + fontSize);
            }
            if (!save) this.drawSelectedDot(context, group, x, y, width, height, border);
        }
        for (let tile of tiles) {
     ...
     *                thicknesses.
     */
    ContentView.prototype.drawTileBorder = function(context, group, tile, border) {
        context.fillStyle = border.color;
        if (group.isTopMost(tile)) {
            // Draw top border.
            context.fillRect(
     ...
     */
    ContentView.prototype.drawSelectedDot = function(context, group, x, y, width, height, border) {
        if (attrs.get(this).selected_view.isSelected(group)) {
            context.fillStyle = "#456CAD";
            context.beginPath();
            var radius = Math.min(5, Math.min(this.scale(width / 2), this.scale(height / 2)));
            context.arc(this.scale(x) + radius * 2, this.scale(y + height) - radius * 2, radius, 0, Math.PI * 2);
            context.fill();
        }
    };
